    ...
    return function ({ addUtilities }) {
        ...
        const utilities = [
            ...
            Object.entries(wordsets).map(([key, wordset]) => {
                const { caretColor = d.caretColor } = wordset;
                return {
            ...
            Object.entries(wordsets).map(([key, wordset]) => {
                const { words, repeat = d.repeat, writeSpeed = d.writeSpeed, eraseSpeed = d.eraseSpeed, pauseBetween = d.pauseBetween } = wordset;
                return generateKeyframes(key, words, writeSpeed, eraseSpeed, pauseBetween, repeat === -1);
            }),
            // Define utility classes
            Object.entries(wordsets).map(([key, wordset]) => {
                const { words, delay = d.delay, repeat = d.repeat, writeSpeed = d.writeSpeed, eraseSpeed = d.eraseSpeed, pauseBetween = d.pauseBetween, caretColor = d.caretColor, caretWidth = d.caretWidth, caretSpacing = d.caretSpacing, blinkSpeed = d.blinkSpeed } = wordset;
...
function generateKeyframes(label, words, writeSpeed, eraseSpeed, pauseBetween, infinite) {
    const keyframes = {
        from: { content: '""' },
    ...
    words.forEach((currentWord) => {
        ...
        writeSteps.forEach((currentStep, i) => {
            if (currentStep) {
        ...
        if (eraseSpeed) {
            ...
            eraseSteps.forEach((currentStep, i) => {
                if (currentStep) {
...
function animationLength(words, writeSpeed, eraseSpeed, pauseBetween) {
    let totalTime = 0;
    words.forEach((currentWord) => {
        totalTime +=
...
function calculateSteps(currentWord, phaseSpeed, totalTime) {
...
function randomize(number, variance = 0.5) {
